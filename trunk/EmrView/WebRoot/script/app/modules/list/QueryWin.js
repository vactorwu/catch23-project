...
Ext.extend(app.modules.list.QueryWin, app.desktop.Module, {
	...
	getHistoryDataFromCookie:function(){
		...
		for(var id in history[hid]){
			...
			his_data.push({name:id,description:history[hid][id],orOrAnd:history[hid][id+"_orOrAnd"]})
		}
		return his_data
	},
...
//		alert(Ext.encode(this.getCookie("queryWin_history")))
		...
		Ext.Msg.prompt("保存","为该查询条件取个名字.",function(btn,txt){
			if(btn == "ok"){
				...
				if(history[hid][txt]){
					Ext.MessageBox.confirm("提示","已存在该查询条件名称，是否覆盖？",function(btn){
						if(btn == 'yes'){
							history[hid][txt] = rs
							history[hid][txt+"_orOrAnd"] = this.orOrAnd.getValue() || 'and'
							this.setCookie("queryWin_history",Ext.encode(history))
							var rnum  = this.HP.getStore().find('name',txt);
							if(rnum > -1){
								var r = this.HP.getStore().getAt(rnum);
								r.set('description', rs);
								r.set('orOrAnd', history[hid][txt+"_orOrAnd"])
							}
						}
					},this)
				}else{
					...
					history[hid][txt+"_orOrAnd"] = this.orOrAnd.getValue() || 'and'
					this.setCookie("queryWin_history",Ext.encode(history))
					this.HP.getStore().add(new this.historyRecord({name:txt,description:rs,orOrAnd:history[hid][txt+"_orOrAnd"]}))
				}
			}
		},this)
	},
