...
namespace Trade.BusinessServiceImplementation
{
    ...
    public class CustomCertValidator : CustomCertificateValidator
    {
        ...
        protected override string[] getAllowedThumbprints()
        {
            ...
            //This is the thumbprint for the Test Certificate CN=StockTraderBSLClient.Com (StockTraderBSLClient.pfx).
            //This Cert is used by the Business Service Layer to validate the StockTrader Client. When using this binding in
            //conjucntion with the service behavior 'BSL_M_Security_ClientCert_Behavior' (which also assigns the service certificate), 
            //only clients presentin the EXACT client certificate will be allowed in.  The thumbprint is unique, meaning, 
            //no one can create another cert with this thumbprint.  Hence, only an app with this exact client-side
